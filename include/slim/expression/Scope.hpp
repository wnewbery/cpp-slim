...
namespace slim { namespace expr
{
    ...
    public:
        ...
        /**Get the "self" variable. Not that unlike get("self") this throws instead of returning
         * NIL_VALUE if the variable was not set.
         *
         * In most situations, "self" should be set.
         */
        ObjectPtr self()
        {
            ...
            if (it != map.end()) return it->second;
            else if (parent) return parent->self();
            else throw std::runtime_error("self not set");
        }
